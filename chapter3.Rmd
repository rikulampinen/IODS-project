...
ggsave("alc_gather.png", 
       plot = alc_gather, path = "C:/Users/richla/OneDrive/1 C - R-Folder/11-IODS-course/IODS-project/data/", scale = 1,
       dpi = 300)
``` 
...
**1. Calculation of the logistic regression model.**  
...
* Being a male student is significantly (p = 5.37e-05 ***) increases the probability of high alcohol consumption by ~1.
...
Here the last 20 data points with the probability and prediction outputs:
...
true high use --> false negative --> 82 --> more important in this case 82 are predicted not do have high alc use but they are having high alcohol consumption


false high use --> false negative --> 15
...
g + geom_point() + xlab("Probability value") + ylab("High alcohol use") + ggtitle("High alcohol use versus probability")
...
[1] 0.2565445 --> every 4th prediction is false --> 25% of predictions are false!
--> percentage of false prediction
...
> cv$delta[1]
[1] 0.2565445 --> percentage of false predictions

cross validation tells us if the model is too much in the data --> if the model is more generalized the value of cross validation shall be higher that the loss function result
