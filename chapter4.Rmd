...
The prediction showed not so good results on the lower crime rate predictions. Results show that the crime rate was predicted higher than it actually is (shows in predicted med_low of 9 and med_high which are actually low rates). In the medium crime level it also wrongly predicted quite a number of values, but on the high crime rates the predictions is quite good with just on value wrongly predicted.
...
```{r, superbonus, echo=TRUE, results='markdown', message=FALSE, warning=FALSE, fig.align="center"}
...
clusterplot <- plot_ly(x = matrix_product$LD1, y = matrix_product$LD2, z = matrix_product$LD3, type= 'scatter3d', mode='markers',color = classes3)


```
  
The 3D plot of the crime training data set is visible here.
The cluster plot does not work... I don't know why...
